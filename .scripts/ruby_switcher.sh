...
function use_ruby_186 {
 ...
 export GEM_HOME=~/.gem/ruby/1.8
 export RUBYLIB=$MY_RUBY_HOME/lib/ruby
 update_path
}
...
function install_gems_from_source {
  local url="http://rubyforge.org/frs/download.php/60718/rubygems-1.3.5.tgz"
  mkdir -p ~/tmp &&
  pushd ~/tmp &&
  curl --silent -L -O $url &&
  tar xzvf rubygems-1.3.5.tgz &&
  cd rubygems-1.3.5 &&
  ruby setup.rb all --prefix=$MY_RUBY_HOME
